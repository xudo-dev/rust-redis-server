...
impl Value {
    pub fn to_command(&self) -> Result<(String, Vec<Value>)> {
        match self {
            ...
            _ => Err(Error::msg("invalid command, not an array")),
        }
    }
    ...
    fn unwrap_bulk(&self) -> String {
        match self {
            ...
            _ => panic!("invalid command, not a bulk string"),
        }
    }
...
fn decode_simple_string(buffer: BytesMut) -> Result<Option<(Value, usize)>> {
    if let Some((line, len)) = read_unitl_crlf(&buffer[1..]) {
...
fn decode_array(buffer: BytesMut) -> Result<Option<(Value, usize)>> {
    let (array_length, mut bytes_consumed) = if let Some((line, len)) = read_unitl_crlf(&buffer[1..]) {
...
fn decode_bulk_string(buffer: BytesMut) -> Result<Option<(Value, usize)>> {
    let (bulk_length, bytes_consumed) = if let Some((line, len)) = read_unitl_crlf(&buffer[1..]) {
...
fn read_unitl_crlf(buffer: &[u8]) -> Option<(&[u8], usize)> {
